...
@Component({
  selector: 'app-coursewebdevlearner',
  templateUrl: './coursewebdevlearner.component.html',
  styleUrls: ['./coursewebdevlearner.component.css']
})
export class CoursewebdevlearnerComponent implements OnInit {
  ...
  updateLoginpage(data:any){
    this.ps1.updateWebDevLearner(data.value).subscribe();
    alert('You Have successfully updated learner data');
    window.location.reload();
  }

  deleteLoginpage(data:any){
    this.ps.deleteWebDevLearner(data.value).subscribe();
  ...
  loginpages:any;
  viewLoginpage(){
    this.is.viewWebDevLearner().subscribe((resp1:any)=>{this.loginpages=resp1;});
  }
