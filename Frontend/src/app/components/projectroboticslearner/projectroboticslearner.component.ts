...
@Component({
  selector: 'app-projectroboticslearner',
  templateUrl: './projectroboticslearner.component.html',
  styleUrls: ['./projectroboticslearner.component.css']
})
export class ProjectroboticslearnerComponent  implements OnInit {
  ...
  updateLoginpage(data:any){
    this.ps1.updateRoboLearner(data.value).subscribe();
    alert('You Have successfully updated learner data');
    window.location.reload();
  }

  deleteLoginpage(data:any){
    this.ps.deleteRoboLearner(data.value).subscribe();
  ...
  loginpages:any;
  viewLoginpage(){
    this.is.viewRoboLearner().subscribe((resp1:any)=>{this.loginpages=resp1;});
  }
