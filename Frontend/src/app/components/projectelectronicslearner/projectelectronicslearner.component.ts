...
@Component({
  selector: 'app-projectelectronicslearner',
  templateUrl: './projectelectronicslearner.component.html',
  styleUrls: ['./projectelectronicslearner.component.css']
})
export class ProjectelectronicslearnerComponent  implements OnInit {
  ...
  updateLoginpage(data:any){
    this.ps1.updateElecLearner(data.value).subscribe();
    alert('You Have successfully updated learner data');
    window.location.reload();
  }

  deleteLoginpage(data:any){
    this.ps.deleteElecLearner(data.value).subscribe();
  ...
  loginpages:any;
  viewLoginpage(){
    this.is.viewElecLearner().subscribe((resp1:any)=>{this.loginpages=resp1;});
  }
