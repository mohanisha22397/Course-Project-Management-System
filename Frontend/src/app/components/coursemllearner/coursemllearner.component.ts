...
@Component({
  selector: 'app-coursemllearner',
  templateUrl: './coursemllearner.component.html',
  styleUrls: ['./coursemllearner.component.css']
})
export class CoursemllearnerComponent  implements OnInit {
  ...
  updateLoginpage(data:any){
    this.ps1.updateMLLearner(data.value).subscribe();
    alert('You Have successfully updated learner data');
    window.location.reload();
  }

  deleteLoginpage(data:any){
    this.ps.deleteMLLearner(data.value).subscribe();
  ...
  loginpages:any;
  viewLoginpage(){
    this.is.viewMLLearner().subscribe((resp1:any)=>{this.loginpages=resp1;});
  }
