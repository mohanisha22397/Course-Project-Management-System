...
@Component({
  selector: 'app-projectiotlearner',
  templateUrl: './projectiotlearner.component.html',
  styleUrls: ['./projectiotlearner.component.css']
})
export class ProjectiotlearnerComponent  implements OnInit {
  ...
  updateLoginpage(data:any){
    this.ps1.updateIOTLearner(data.value).subscribe();
    alert('You Have successfully updated learner data');
    window.location.reload();
  }

  deleteLoginpage(data:any){
    this.ps.deleteIOTLearner(data.value).subscribe();
